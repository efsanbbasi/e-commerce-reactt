...
import { useGetAllCategoryQuery, useGetAllProductsQuery } from "../../store/features/product/productSlice";

const Category = () => {
    const {data} = useGetAllCategoryQuery("");
    var {selectedCtgUrl} = useGetAllProductsQuery("")
        ...
        <div className="category-cont">
           {
            data && data.map((item, index) => (
                <ul className="ctg-item" key={index +1}>
                    <li className="ctg-item-01">
                        <input name={item} className="ctg-item-01-A" type="checkbox" id="ctg-sbox"></input>
                        <label className="ctg-item-01-B" htmlFor="ctg-sbox">{item}</label>
                    </li>
                </ul>
            ))
           }
